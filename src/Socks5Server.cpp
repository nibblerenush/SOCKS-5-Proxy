...
socks5::Socks5Server::Socks5Server(ba::io_context & ioContext, const bpt::ptree & configurationFile):
  ...
  _bufferSize(_configurationFile.get<uint16_t>("buffer.size", DEFAULT_BUFFER_SIZE)),
  _sessionIdCounter(0)
{
  ...
  (
    [this](boost::system::error_code errorCode, ba::ip::tcp::socket acceptSocket)
    {
      if (!errorCode)
      {
        try
        {
          Socks5SessionFactory::CreateSession(std::move(acceptSocket), _bufferSize, _sessionIdCounter++)->Start();
        }
